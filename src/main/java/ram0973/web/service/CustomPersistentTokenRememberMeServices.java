...
import ram0973.web.model.PersistentToken;
import ram0973.web.repository.CustomPersistentTokenRepository;
...
@Service
@Log4j2
public class CustomPersistentTokenRememberMeServices extends AbstractRememberMeServices {
    private static final int TOKEN_VALIDITY_DAYS = 31 * 6; // Token is valid for six months
    ...
    private final DetailService personDetailsService;
    ...
    protected CustomPersistentTokenRememberMeServices(
        DetailService personDetailsService,
        CustomPersistentTokenRepository customPersistentTokenRepository, PersonRepository personRepository) {
        super(rememberMeKey, personDetailsService);
    ...
    protected UserDetails processAutoLoginCookie(String[] cookieTokens, HttpServletRequest request, HttpServletResponse response) {
        synchronized (this) { // prevent 2 authentication requests from the same user in parallel
            ...
            if (login == null) {
                ...
                login = token.getPerson().getEmail();
                ...
                try {
                    customPersistentTokenRepository.saveAndFlush(token);
    ...
    protected void onLoginSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        PersistentToken token = personRepository
    ...
    public void logout(HttpServletRequest request, HttpServletResponse response, Authentication authentication) {
        ...
        if (rememberMeCookie != null && rememberMeCookie.length() != 0) {
            try {
                ...
                PersistentToken token = getPersistentToken(cookieTokens);
                customPersistentTokenRepository.deleteById(token.getSeries());
            } catch (InvalidCookieException ex) {
                log.info("Invalid cookie, no persistent token could be deleted", ex);
            } catch (RememberMeAuthenticationException ex) {
                log.debug("No persistent token found, so no token could be deleted", ex);
            }
        }
